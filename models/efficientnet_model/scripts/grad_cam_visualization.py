...
def load_and_preprocess_image(img_path, target_size=IMAGE_SIZE):
    """
    Load and preprocess a single image for prediction with EfficientNetV2-S.
...
def make_gradcam_heatmap(img_array, model, last_conv_layer_name, pred_index=None):
    ...
    # Get the base model (EfficientNetV2-S)
    base_model = None
    ...
    if base_model is None:
        print("Could not find base model in the loaded model. Using the main model instead.")
        base_model = model
    
    # Find the last convolutional layer
    last_conv_layer = None
    if base_model == model:
        # Search in the main model
    ...
    # Create a model that maps the input image to the activations of the last conv layer and model's output
    if base_model == model:
...
def apply_gradcam(image_path, model_path, output_path=None):
    """
    ...
    # Find the base model (EfficientNetV2-S)
    base_model = None
    for layer in model.layers:
        if hasattr(layer, 'layers') and len(layer.layers) > 0:
            base_model = layer
    ...
    with tf.GradientTape() as tape:
        ...
        if base_model == model:
            last_conv_output = model.get_layer(layer_name).output
            last_conv_model = tf.keras.models.Model(model.inputs, last_conv_output)
        else:
            last_conv_output = base_model.get_layer(layer_name).output
...
def process_multiple_images(image_paths, model_path, output_dir):
    """
    ...
    for i, img_path in enumerate(image_paths):
        ...
        # Apply Grad-CAM
        try:
            apply_gradcam(img_path, model_path, output_path)
...
def main():
    ...
    # Create output directory if it doesn't exist
    output_dir = "models/efficientnet_model/results/figures/grad_cam"
    os.makedirs(output_dir, exist_ok=True)
    ...
    # Process the images
    process_multiple_images(image_paths, MODEL_PATH, output_dir)
